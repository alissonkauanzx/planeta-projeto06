...
interface CurrentUserType extends FirebaseUser {
  isAdmin?: boolean; // Adicionaremos lógica de admin futuramente se necessário
}

interface Project {
  id: string | number; // Supabase pode gerar bigint ou uuid
  title: string;
  description: string;
  category: string;
  ...
  pdf_urls?: string[];
  ods?: number[];
  views?: number; // Adicionar se for usar
  comments?: Comment[]; // Comentários carregados separadamente
  // Outros campos que vierem do Supabase
  [key: string]: any; // Para campos adicionais como 'content' se ainda existir
}
...
interface ProjectsViewProps {
  ...
  odsOptions: any[]; // Tipar melhor se necessário
}

interface CreateProjectViewProps {
  currentUser: CurrentUserType | null; // Necessário para author_id
  handleLogout: () => Promise<void>;
  setCurrentView: (view: string) => void;
  ...
  setLoading: (loading: boolean) => void; // Para o estado de loading geral
  loading: boolean; // Para desabilitar botões
  odsOptions: any[];
}
...
interface ProjectDetailViewProps {
  ...
  setCurrentView: (view: string) => void;
  canEditProject: (project: Project) => boolean;
  handleEditProject: (project: Project) => Promise<void>; // Renomeado para clareza
  handleDeleteProject: (project: Project) => Promise<void>; // Renomeado para clareza
...
// --- Componentes de UI Auxiliares (Movidos para o topo) ---
...
const OptimizedSpaceBackground: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at 20% 20%, rgba(30, 41, 59, 0.9) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(67, 56, 202, 0.8) 0%, transparent 50%), linear-gradient(135deg, #0f172a 0%, #1e1b4b 25%, #312e81 50%, #1e1b4b 75%, #0f172a 100%)`}}/>
      <div className="absolute opacity-80" style={{ top: "15%", right: "10%", width: "200px", height: "200px", background: `radial-gradient(circle at center, transparent 30%, rgba(139, 92, 246, 0.2) 40%, rgba(59, 130, 246, 0.3) 50%, rgba(30, 41, 59, 0.8) 70%, transparent 90%)`, borderRadius: "50%", willChange: "transform", animation: "simpleRotate 20s linear infinite", }}>
        <div className="absolute top-1/2 left-1/2" style={{ width: "40px", height: "40px", background: "radial-gradient(circle, #000000 0%, rgba(0,0,0,0.8) 80%, transparent 100%)", borderRadius: "50%", transform: "translate(-50%, -50%)", }} />
      </div>
      <div className="absolute inset-0 opacity-20" style={{ background: `radial-gradient(ellipse 600px 300px at 30% 70%, rgba(139, 92, 246, 0.3) 0%, transparent 60%), radial-gradient(ellipse 400px 200px at 70% 30%, rgba(59, 130, 246, 0.25) 0%, transparent 60%)`, willChange: "transform", animation: "gentleFloat 30s ease-in-out infinite alternate", transform: `translate3d(${mousePosition.x * 0.01}px, ${mousePosition.y * 0.01}px, 0)`, }} />
      {[...Array(50)].map((_, i) => {
        const size = Math.random() * 2 + 1; const brightness = Math.random() * 0.8 + 0.2; const delay = Math.random() * 5;
        return <div key={i} className="absolute rounded-full" style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, width: `${size}px`, height: `${size}px`, backgroundColor: "#ffffff", opacity: brightness, willChange: "opacity", animation: `gentleTwinkle ${3 + Math.random() * 2}s ease-in-out infinite`, animationDelay: `${delay}s`, }} />
      })}
      <svg className="absolute inset-0 w-full h-full opacity-30" style={{ pointerEvents: "none" }}>
        <g opacity="0.6"><line x1="15%" y1="25%" x2="20%" y2="30%" stroke="rgba(59, 130, 246, 0.5)" strokeWidth="1" /><line x1="20%" y1="30%" x2="25%" y2="25%" stroke="rgba(59, 130, 246, 0.5)" strokeWidth="1" /><line x1="25%" y1="25%" x2="30%" y2="35%" stroke="rgba(59, 130, 246, 0.5)" strokeWidth="1" /><circle cx="15%" cy="25%" r="2" fill="rgba(255, 255, 255, 0.8)" /><circle cx="20%" cy="30%" r="3" fill="rgba(59, 130, 246, 0.8)" /><circle cx="25%" cy="25%" r="2" fill="rgba(255, 255, 255, 0.8)" /><circle cx="30%" cy="35%" r="2" fill="rgba(147, 51, 234, 0.8)" /></g>
        <g opacity="0.5"><line x1="70%" y1="20%" x2="75%" y2="15%" stroke="rgba(147, 51, 234, 0.5)" strokeWidth="1" /><line x1="75%" y1="15%" x2="80%" y2="25%" stroke="rgba(147, 51, 234, 0.5)" strokeWidth="1" /><circle cx="70%" cy="20%" r="2" fill="rgba(255, 255, 255, 0.7)" /><circle cx="75%" cy="15%" r="2" fill="rgba(147, 51, 234, 0.8)" /><circle cx="80%" cy="25%" r="2" fill="rgba(255, 255, 255, 0.7)" /></g>
      </svg>
      <div className="absolute opacity-60" style={{ left: "10%", top: "20%", willChange: "transform", animation: "simpleMeteor 12s linear infinite", }} > <div className="w-1 h-20 bg-gradient-to-b from-white via-blue-300 to-transparent" style={{ transform: "rotate(45deg)", filter: "blur(0.5px)", }} /> </div>
      <div className="absolute inset-0">
        <div className="absolute top-32 left-16 w-32 h-32 rounded-full opacity-70" style={{ background: `radial-gradient(circle at 30% 30%, #60a5fa 0%, #3b82f6 50%, #1e40af 100%)`, willChange: "transform", animation: "slowRotate 40s linear infinite", }} > <div className="absolute rounded-full border opacity-40" style={{ width: "160%", height: "160%", top: "-30%", left: "-30%", borderColor: "rgba(59, 130, 246, 0.3)", borderWidth: "1px", transform: "rotateX(75deg)", }} /> </div>
        <div className="absolute bottom-40 right-32 w-20 h-20 rounded-full opacity-60" style={{ background: `radial-gradient(circle at 40% 40%, #f472b6 0%, #ec4899 50%, #be185d 100%)`, willChange: "transform", animation: "slowRotate 50s linear infinite reverse", }} />
      </div>
      ...
      <div className="absolute inset-0 opacity-10 transition-opacity duration-500" style={{ background: `radial-gradient(circle at ${mousePosition.x}% ${mousePosition.y}%, rgba(59, 130, 246, 0.2) 0%, transparent 50%)`, }} />
      <style jsx>{` @keyframes simpleRotate { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } @keyframes slowRotate { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } @keyframes gentleFloat { 0%, 100% { transform: translate3d(0, 0, 0) scale(1); } 50% {  transform: translate3d(10px, -10px, 0) scale(1.05); } } @keyframes gentleTwinkle { 0%, 100% { opacity: 0.2; transform: scale(1); } 50% { opacity: 0.8; transform: scale(1.2); } } @keyframes simpleMeteor { 0% { transform: translate3d(-50px, -50px, 0) rotate(45deg); opacity: 0; } 10% { opacity: 0.6; } 90% { opacity: 0.6; } 100% { transform: translate3d(100vw, 100vh, 0) rotate(45deg); opacity: 0; } } * { backface-visibility: hidden; perspective: 1000px; } `}</style>
    </div>
  )
}
...
const OptimizedCard: React.FC<OptimizedCardProps> = ({ children, className = "", onClick }) => {
  ...
  return (
    <div className={`relative ${className} cursor-pointer transition-all duration-300 transform hover:scale-105 hover:-translate-y-2`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onClick={onClick} style={{ willChange: "transform", }} >
      {isHovered && ( <div className="absolute inset-0 rounded-lg pointer-events-none"> <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-transparent via-blue-400 to-transparent opacity-60" /> <div className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-transparent via-purple-400 to-transparent opacity-60" /> <div className="absolute left-0 top-0 w-0.5 h-full bg-gradient-to-b from-transparent via-blue-400 to-transparent opacity-60" /> <div className="absolute right-0 top-0 w-0.5 h-full bg-gradient-to-b from-transparent via-purple-400 to-transparent opacity-60" /> </div> )}
      {children}
    </div>
  )
}

// --- Componentes de View (Movidos para fora) ---
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", }} >
      <div style={{ width: "100%", maxWidth: "400px", backgroundColor: "rgba(15, 23, 42, 0.95)", padding: "30px", borderRadius: "15px", border: "1px solid rgba(59, 130, 246, 0.3)", backdropFilter: "blur(20px)", boxShadow: "0 25px 50px rgba(0, 0, 0, 0.5), 0 0 50px rgba(59, 130, 246, 0.1)", }} >
        ...
        <div style={{ marginBottom: "20px" }}>
          <label style={{ display: "block", color: "#e2e8f0", marginBottom: "8px", fontSize: "14px", fontWeight: "500", }} > Email </label>
          <input type="email" placeholder="seu@email.com" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} style={{ width: "100%", padding: "14px", backgroundColor: "rgba(51, 65, 85, 0.8)", border: "1px solid rgba(71, 85, 105, 0.8)", borderRadius: "8px", color: "white", fontSize: "16px", outline: "none", boxSizing: "border-box", transition: "all 0.3s ease", }} />
        </div>
        <div style={{ marginBottom: "30px" }}>
          <label style={{ display: "block", color: "#e2e8f0", marginBottom: "8px", fontSize: "14px", fontWeight: "500", }} > Senha </label>
          <input type="password" placeholder="••••••••" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} style={{ width: "100%", padding: "14px", backgroundColor: "rgba(51, 65, 85, 0.8)", border: "1px solid rgba(71, 85, 105, 0.8)", borderRadius: "8px", color: "white", fontSize: "16px", outline: "none", boxSizing: "border-box", transition: "all 0.3s ease", }} />
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <button onClick={() => setCurrentView("register")} style={{ background: "none", border: "none", color: "#60a5fa", fontSize: "14px", cursor: "pointer", textDecoration: "underline", transition: "color 0.3s ease", }} > Não tem conta? Criar conta </button>
        </div>
      </div>
    </div>
);
...
const RegisterView: React.FC<RegisterViewProps> = ({ registerEmail, setRegisterEmail, registerPassword, setRegisterPassword, handleRegister, setCurrentView, loading }) => (
  <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", }} >
    <div style={{ width: "100%", maxWidth: "400px", backgroundColor: "rgba(15, 23, 42, 0.95)", padding: "30px", borderRadius: "15px", border: "1px solid rgba(139, 92, 246, 0.3)", backdropFilter: "blur(20px)", boxShadow: "0 25px 50px rgba(0, 0, 0, 0.5), 0 0 50px rgba(139, 92, 246, 0.1)", }} >
      ...
      <div style={{ marginBottom: "20px" }}>
        <label style={{ display: "block", color: "#e2e8f0", marginBottom: "8px", fontSize: "14px", fontWeight: "500", }} > Email </label>
        <input type="email" placeholder="seu@email.com" value={registerEmail} onChange={(e) => setRegisterEmail(e.target.value)} style={{ width: "100%", padding: "14px", backgroundColor: "rgba(51, 65, 85, 0.8)", border: "1px solid rgba(71, 85, 105, 0.8)", borderRadius: "8px", color: "white", fontSize: "16px", outline: "none", boxSizing: "border-box", transition: "all 0.3s ease", }} />
      </div>
      <div style={{ marginBottom: "30px" }}>
        <label style={{ display: "block", color: "#e2e8f0", marginBottom: "8px", fontSize: "14px", fontWeight: "500", }} > Senha </label>
        <input type="password" placeholder="••••••••" value={registerPassword} onChange={(e) => setRegisterPassword(e.target.value)} style={{ width: "100%", padding: "14px", backgroundColor: "rgba(51, 65, 85, 0.8)", border: "1px solid rgba(71, 85, 105, 0.8)", borderRadius: "8px", color: "white", fontSize: "16px", outline: "none", boxSizing: "border-box", transition: "all 0.3s ease", }} />
      </div>
      ...
      <div style={{ textAlign: "center" }}>
        <button onClick={() => setCurrentView("login")} style={{ background: "none", border: "none", color: "#a78bfa", fontSize: "14px", cursor: "pointer", textDecoration: "underline", transition: "color 0.3s ease", }} > Já tem conta? Entrar </button>
      </div>
    </div>
  </div>
);
...
const ProjectsView: React.FC<ProjectsViewProps> = ({ currentUser, projects, filteredProjects, searchTerm, setSearchTerm, filterCategory, setFilterCategory, categories, handleLogout, setCurrentView, setSelectedProject, canEditProject, odsOptions }) => (
  // ... (Conteúdo do ProjectsView como antes, mas agora recebe props)
  // Substituir chamadas diretas a estados/funções do PlanetaProjeto por props. Ex: currentUser em vez de state.currentUser
  // Exemplo: Em vez de {currentUser?.isAdmin && ...}, usar {props.currentUser?.isAdmin && ...}
  // O conteúdo interno do return JSX permanece o mesmo, apenas o acesso às variáveis muda para props.
          ...
          <div className="flex items-center justify-between min-h-[48px]">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* {currentUser?.isAdmin && (...)}  Lógica de admin será via props.currentUser se necessário */}
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Button onClick={() => setCurrentView("create")} className="relative overflow-hidden group h-12 px-6" style={{ background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", border: "none", boxShadow: "0 0 20px rgba(59, 130, 246, 0.4)", transition: "all 0.3s ease", }} > <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" /> <Plus className="mr-2 h-5 w-5" /> Postar Projeto </Button>
              <Button onClick={handleLogout} variant="outline" className="relative overflow-hidden group bg-transparent h-12 px-6" style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.4)", color: "#fca5a5", boxShadow: "0 0 15px rgba(239, 68, 68, 0.2)", transition: "all 0.3s ease", minWidth: "100px", }} > <LogOut className="mr-2 h-4 w-4" /> Sair </Button>
            </div>
          </div>
        </div>
      </header>
      <div className="p-6 pt-12">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-5xl font-bold text-white mb-6 relative" style={{ textShadow: "0 0 30px rgba(59, 130, 246, 0.5)", }} > Projetos Espaciais <div className="absolute inset-0 bg-gradient-to-r from-transparent via-blue-300/20 to-transparent" style={{ animation: "titleShine 3s ease-in-out infinite", backgroundSize: "200% 100%", }} /> </h2>
...
const CreateProjectView: React.FC<CreateProjectViewProps> = ({ currentUser, handleLogout, setCurrentView, uploadToCloudinary, uploadToSupabaseStorage, loadProjectsFromSupabase, setLoading, loading: parentLoading, odsOptions }) => {
  // ... (Conteúdo do CreateProjectView como antes, mas agora recebe props)
  // A lógica de estado de projectData, uploading, refs de input permanecem internas a esta View.
  // Funções como handleFileUpload, removeFile, handleSubmit também permanecem aqui, mas usam props como currentUser, setLoading, etc.
  ...
  const handleSubmit = async () => {
    ...
    if (!currentUser) { alert("Usuário não autenticado. Por favor, faça login novamente."); setCurrentView("login"); return; }
    setLoading(true);
...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ selectedProject, setSelectedProject, currentUser, newComment, setNewComment, setCurrentView, canEditProject, handleEditProject, handleDeleteProject, handleAddComment, setLoading, odsOptions }) => {
  // ... (Conteúdo do ProjectDetailView como antes, mas agora recebe props)
  // A lógica de estado para `newComment` pode permanecer aqui ou ser elevada se necessário.
  // Funções como handleAddComment, handleEdit, handleDelete serão chamadas a partir das props.
  // O useEffect para carregar comentários permanece aqui, usando selectedProject.id da prop.

  // O JSX do ProjectDetailView permanece o mesmo, mas usa props para dados e funções
  if (!selectedProject) return null;
  const canEdit = canEditProject(selectedProject); // Usa a prop

  // useEffect para carregar comentários quando um projeto é selecionado
  useEffect(() => {
...
export default function PlanetaProjeto() {
  ...
  const canEditProject = useCallback((project: Project) => {
    ...
    return currentUser.uid === project.author_id;
  }, [currentUser]);
  ...
  const uploadToCloudinary = async (file: File): Promise<string | null> => {
    if (!file || !CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) {
      console.error("Cloudinary config is missing or file not provided.");
      alert("Erro na configuração do upload de imagem.");
      return null;
    }
  ...
  const uploadToSupabaseStorage = async (file: File, type: 'videos' | 'pdfs'): Promise<string | null> => {
    ...
    } catch (e: any) { console.error(`Supabase storage upload error (${type}):`, e); alert(`Erro ao enviar ${type}: ${e.message}`); return null; }
  };
  ...
  }, [newComment, currentUser, selectedProject, setSelectedProject, setNewComment, setLoading]);

  const handleEditProjectDetail = useCallback(async (project: Project) => {
      if (!currentUser || currentUser.uid !== project.author_id) { alert("Não autorizado."); return; }
      ...
      setLoading(true);
      try {
        const { data, error } = await supabase.from('projetos').update({ title: newTitle, description: newDesc }).eq('id', project.id).eq('author_id', currentUser.uid).select().single();
        if (error) throw error;
  ...
  }, [currentUser, loadProjectsFromSupabase, setLoading, setSelectedProject]);
  ...
  const handleDeleteProjectDetail = useCallback(async (project: Project) => {
    ...
    if (confirm(`Apagar "${project.title}"?`)) {
      setLoading(true);
      try {
        // TODO: Deletar comentários e arquivos de storage associados
        const { error } = await supabase.from('projetos').delete().eq('id', project.id).eq('author_id', currentUser.uid);
        if (error) throw error;
  ...
  },[currentUser, loadProjectsFromSupabase, setCurrentView, setLoading, setSelectedProject]);

  if (loading && (currentView === "login" || currentView === "register" || !currentUser && currentView !== 'login' && currentView !== 'register')) {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      {/* Fallback para login se nenhuma view corresponder e não estiver logado */}
      {!isLoggedIn && !["login", "register"].includes(currentView) && <LoginView {...{ loginEmail, setLoginEmail, loginPassword, setLoginPassword, handleLogin, setCurrentView, loading }} />}
...
const ProjectsView: React.FC<ProjectsViewProps> = ({ currentUser, projects, filteredProjects, searchTerm, setSearchTerm, filterCategory, setFilterCategory, categories, handleLogout, setCurrentView, setSelectedProject, canEditProject, odsOptions }) => (
  <div className="min-h-screen relative z-10">
      <header className="sticky top-0 z-50 relative overflow-hidden" style={{ background: "rgba(15, 23, 42, 0.95)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(59, 130, 246, 0.3)", boxShadow: "0 10px 30px rgba(0, 0, 0, 0.3)", minHeight: "80px", }} >
        <div className="absolute inset-0 opacity-10" style={{ background: `linear-gradient(90deg, transparent, rgba(59, 130, 246, 0.1), transparent)`}} />
        <div className="container mx-auto px-4 py-4 relative"> <div className="flex items-center justify-between min-h-[48px]"> <div className="flex items-center gap-4"> <div className="relative p-3 rounded-full" style={{ background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", boxShadow: "0 0 30px rgba(59, 130, 246, 0.5)", }} > <Planet className="h-8 w-8 text-white animate-spin" style={{ animationDuration: "10s" }} /> {[...Array(6)].map((_, i) => ( <div key={i} className="absolute w-2 h-2 bg-blue-300 rounded-full" style={{ top: `${Math.sin((i * Math.PI) / 3) * 25 + 50}%`, left: `${Math.cos((i * Math.PI) / 3) * 25 + 50}%`, animation: `orbit ${3 + i * 0.3}s linear infinite`, transformOrigin: "50% 50%", }} /> ))} </div> <div className="flex items-center gap-3"> <h1 className="text-2xl font-bold text-white"> Planeta Projeto{" "} <span className="inline-block animate-pulse" style={{ animationDelay: "1s" }}> 🌎 </span> </h1> {/* Admin Badge Logic */} </div> </div> <div className="flex items-center gap-3"> <Button onClick={() => setCurrentView("create")} className="relative overflow-hidden group h-12 px-6" style={{ background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", border: "none", boxShadow: "0 0 20px rgba(59, 130, 246, 0.4)", transition: "all 0.3s ease", }} > <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" /> <Plus className="mr-2 h-5 w-5" /> Postar Projeto </Button> <Button onClick={handleLogout} variant="outline" className="relative overflow-hidden group bg-transparent h-12 px-6" style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.4)", color: "#fca5a5", boxShadow: "0 0 15px rgba(239, 68, 68, 0.2)", transition: "all 0.3s ease", minWidth: "100px", }} > <LogOut className="mr-2 h-4 w-4" /> Sair </Button> </div> </div> </div> </header>
...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ selectedProject, setSelectedProject, currentUser, newComment, setNewComment, setCurrentView, canEditProject, handleEditProject, handleDeleteProject, handleAddComment, setLoading, odsOptions }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      if (selectedProject?.id) {
        // Idealmente, usar um estado de loading específico para comentários aqui
        // setLoading(true);
        try {
          const { data: commentsData, error } = await supabase.from('comentarios').select('*').eq('project_id', selectedProject.id).order('created_at', { ascending: true });
          if (error) throw error;

          // A prop setSelectedProject deve ser capaz de aceitar uma função para atualizar o estado baseado no anterior
...
export default function PlanetaProjeto() {
  ...
  const loadProjectsFromSupabase = useCallback(async () => {
    setLoading(true);
    try {
      const { data: projectData, error: projectError } = await supabase.from('projetos').select('*').order('created_at', { ascending: false });
      if (projectError) throw projectError;
      setProjects(projectData || []);
    } catch (error: any) {
      console.error("Erro ao buscar projetos:", error);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      // setLoading(true); // Movido para dentro do if/else para evitar piscar
      if (user) {
        setCurrentUser(user as CurrentUserType);
        setIsLoggedIn(true);
        if (currentView === "login" || currentView === "register") { // Evitar mudar a view se já estiver em uma view de projeto
            setCurrentView("projects");
        }
        loadProjectsFromSupabase();
      } else {
      ...
      setLoading(false); // setLoading aqui depois de todas as operações de auth
    });
    return () => unsubscribe();
  }, [loadProjectsFromSupabase, currentView]); // Adicionado currentView para reavaliar se necessário
  ...
  const handleLogout = useCallback(async () => {
    setLoading(true);
    try { await signOut(auth); setSelectedProject(null); /* Limpar projeto selecionado */ }
    catch (e: any) { alert("Erro ao sair: " + e.message); }
    finally { setLoading(false); }
  }, []);
  ...
  const handleAddCommentProjectDetail = useCallback(async () => {
      if (!newComment.trim() || !currentUser || !selectedProject) { alert("Login e comentário são necessários."); return; }
      const payload = { project_id: selectedProject.id, user_id: currentUser.uid, author_name: currentUser.displayName || currentUser.email?.split('@')[0] || "Anônimo", content: newComment };
      // setLoading(true); // Usar um loading específico para comentários se tiver
      try {
        const { data: savedComment, error } = await supabase.from('comentarios').insert([payload]).select().single();
        if (error) throw error;
        setSelectedProject(prev => prev ? ({ ...prev, comments: [...(prev.comments || []), savedComment] }) : null);
      ...
      // finally { setLoading(false); }
  }, [newComment, currentUser, selectedProject, setSelectedProject, setNewComment]); // Removido setLoading daqui
  ...
  // Loading inicial enquanto o estado de auth não foi determinado
