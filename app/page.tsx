...
interface Project {
  ...
  author_name?: string;
  createdAt: string;
  // Corrigido para corresponder à sua listagem de colunas
  category?: string; // Opcional para evitar erros se a coluna não existir imediatamente
  imageUrl?: string | null;
  videoUrl?: string[]; // Supabase armazena array de texto como text[]
  pdfUrl?: string[];   // Supabase armazena array de texto como text[]
  ods?: number[];
  views?: number;
  comments?: Comment[];
  [key: string]: any;
...
const CreateProjectView: React.FC<CreateProjectViewProps> = ({ currentUser, handleLogout, setCurrentView, uploadToCloudinary, uploadToSupabaseStorage, loadProjectsFromSupabase, setLoading, loading: parentLoading, odsOptions }) => {
  ...
  const handleSubmit = async () => {
    ...
    const newProjectPayload = {
      ...
      imageUrl: projectData.images.length > 0 ? projectData.images[0].url : null,
      videoUrl: projectData.videos.map((v: any) => v.url),
      pdfUrl: projectData.pdfs.map((p: any) => p.url),
    };
